...
function isDriveFilePreviewUrl(url = location.href) {
  ...
  return /https:\/\/drive\.google\.com\/file\/d\/[^/]+\/(view|preview)/.test(
    url
  );
}
...
function findLargestMediaElement() {
  ...
  for (const selector of selectors) {
    ...
    const filtered = elements.filter((el) => {
      ...
      log(
        `  Element: ${el.tagName}, rect: ${rect.width}x${rect.height}, src: ${
          el.src || "none"
      ...
      if (
        ...
        (el.src.includes("icon") ||
          el.src.includes("thumbnail") ||
          el.src.includes("favicon"))
      ) {
        return false;
      }
  ...
  if (allCandidates.length === 0) {
    ...
    allMedia.forEach((el, i) => {
      const rect = el.getBoundingClientRect();
      log(
        `  [${i}] ${el.tagName}: ${rect.width}x${rect.height}, src: ${
          el.src || "none"
  ...
  log(
    `Selected media element: ${selected.tagName} (${rect.width}x${
      rect.height
    }) src: ${selected.src || "no src"}`
  );
  return selected;
}
...
function applyTransform() {
  ...
  log(`Applied rotation: angle=${angle}°, flipX=${flipX}`);
}

function setTarget(el) {
  if (!el) return;
  console.log("setting target", el);
  // If it's already a div with role="img", use it directly
  if (el.matches('div[role="img"]')) {
    currentTarget = el;
  } else {
    // Otherwise, climb up until we find its parent div with role="img"
    currentTarget = el.closest('div[role="img"]');
  }
  // Only continue if we found the correct parent div
  if (!currentTarget) return;
  applyTransform();
}
...
function ensureToolbar() {
  ...
  bar.addEventListener("click", (e) => {
    ...
    // Fix the rotation logic in the click handler (around line 258-259)
    if (action === "left") {
      angle = (angle - 90) % 360;
...
async function initOnDrive() {
  ...
  const assignTarget = debounce(() => {
    ...
    if (el) {
      if (!currentTarget || !currentTarget.contains(el)) {
        log("Found new media element, setting as target");
        setTarget(el); // use this to ensure that the parent div is being selected
      } else if (!el) {
        if (initAttempts <= 3) {
